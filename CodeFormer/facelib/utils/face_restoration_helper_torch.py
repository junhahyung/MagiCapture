...
class FaceRestoreHelper(object):
    ...
    def get_face_landmarks_5(self,
        ...
        if resize is None:
            ...
            h, w = self.input_img.shape[1:3]
            scale = resize / min(h, w)
            scale = max(1, scale) # always scale up
            h, w = int(h * scale), int(w * scale)
            interp = 'area' if scale < 1 else 'linear'
            input_img = F.interpolate(self.input_img, (w, h), mode=interp)
        """

        if resize is None:
            scale = 1
            input_img = self.input_img_numpy
        else:
            h, w = self.input_img_numpy.shape[0:2]
            scale = resize / min(h, w)
            scale = max(1, scale) # always scale up
            h, w = int(h * scale), int(w * scale)
            interp = cv2.INTER_AREA if scale < 1 else cv2.INTER_LINEAR
            input_img = cv2.resize(self.input_img_numpy, (w, h), interpolation=interp)

        with torch.no_grad(): # 아마 이건 no_grad 해도 괜찮을듯? bbox 찾는게 고작이니
        ...
        # pad blurry images
        # this is not the default setting... I'll not edit this, for now...
        """
        if self.pad_blur:
            self.pad_input_imgs = []
    ...
    def align_warp_face(self, save_cropped_path=None, border_mode='constant'):
        ...
        for idx, landmark in enumerate(self.all_landmarks_5):
            ...
            affine_matrix = torch.tensor(cv2.estimateAffinePartial2D(landmark, self.face_template, method=cv2.LMEDS)[0]).to(device=self.input_img.device, dtype=torch.float32)
            self.affine_matrices.append(affine_matrix)
            # warp and crop faces
            if border_mode == 'constant':
                border_mode = 'zeros'
            elif border_mode == 'reflect101':
                border_mode = 'reflection' # in korina.warp_affine, there is no mode 'reflect101'
            ...
            if self.pad_blur:
                ...
                input_img = self.input_img
                # cur input img: BGR, (0, 255), C, H, W
            cropped_face = warp_affine(
                input_img.unsqueeze(0), affine_matrix.unsqueeze(0), self.face_size, padding_mode=border_mode)[0]  # border value 설정이 불가능한데, 이게 큰 영향을 줄까? 
                # ㅇㅇ. 결과가 완전히 변함. 이거 해결해야 할듯
            h, w = cropped_face.shape[1:3]
            cropped_face = cropped_face.permute(1, 2, 0).reshape((-1, 3))
            #print(cropped_face[:, :].shape)
            black_idx = (cropped_face.sum(dim=1) == 0)
            gray_pixel =  torch.tensor([135, 133, 132]).to(device=cropped_face.device, dtype=cropped_face.dtype)
            #print(black_idx.shape)
            cropped_face[black_idx] = gray_pixel # gray
            cropped_face = cropped_face.reshape(h, w, -1)
            cropped_face = cropped_face.permute(2, 0, 1)
